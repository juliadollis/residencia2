...
_ensure(["protobuf", "sentencepiece", "Pillow"])
...
HUB_MODEL_ID = "juliadollis/siglip-finetuned-artdatasets"
OUTPUT_DIR = "./siglip-finetuned-artdatasets"
BATCH_SIZE = 64
SEED = 42
...
if ds_artbench:
    ds_artbench = ds_artbench.map(lambda x: normalize(x, "prompt"), num_proc=NUM_WORKERS)
...
if ds_wikiart:
    ds_wikiart = ds_wikiart.map(lambda x: normalize(x, "description"), num_proc=NUM_WORKERS)
...
if ds_gpt4o:
    ds_gpt4o = ds_gpt4o.map(lambda x: normalize(x, "prompt"), num_proc=NUM_WORKERS)
